...
func TestReader(t *testing.T) {
	...
	// 示例1。
	fmt.Println("New a string reader and name it \"reader1\" ...") //New a string reader and name it "reader1" ...
	reader1 := strings.NewReader(comment)
	buf1 := make([]byte, 7)
	n, err := reader1.Read(buf1)
	var offset1, index1 int64
	executeIfNoErr(err, func() {
		fmt.Printf("Read(%d): %q\n", n, buf1[:n]) //Read(7): "Package"
		offset1 = int64(53)
		index1, err = reader1.Seek(offset1, io.SeekCurrent)
	})
	executeIfNoErr(err, func() {
		fmt.Printf("The new index after seeking from current with offset %d: %d\n",
			offset1, index1) //The new index after seeking from current with offset 53: 60
		n, err = reader1.Read(buf1)
	})
	executeIfNoErr(err, func() {
		fmt.Printf("Read(%d): %q\n", n, buf1[:n]) //Read(7): "primary"
	})
	fmt.Println()
